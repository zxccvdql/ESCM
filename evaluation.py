...
class SceneNetEval():
    ...
    ################################################################################################
    def get_final_metrics(self, net, test_loader, num_gpu = 1, local_rank = 0):
        val_metrics = {}
        batch_size = []
        self.reset_records()
        # Get the predictions
...
#                 gt_batch = test_dataset[i]
        ...
        if 'seg' in self.tasks:
            ...
            for i in range(self.seg_num_class):
                if not self.records['seg']['conf_mat'][i, i] == 0:
                    jaccard_perclass.append(self.records['seg']['conf_mat'][i, i] / (np.sum(self.records['seg']['conf_mat'][i, :]) + np.sum(self.records['seg']['conf_mat'][:, i]) - self.records['seg']['conf_mat'][i, i]))

            # val_metrics_sm = torch.tensor(np.sum(jaccard_perclass) / len(jaccard_perclass)).to('cuda')
            val_metrics['seg']['mIoU'] = np.sum(jaccard_perclass) / len(jaccard_perclass)
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_metrics_sm, 0, op=dist.ReduceOp.SUM)
            #     val_metrics_sm /= int(num_gpu)
            # val_metrics['seg']['mIoU'] = float(val_metrics_sm)
            
            # val_metrics_sp = torch.tensor((np.array(self.records['seg']['pixelAccs']) * np.array(batch_size)).sum() / sum(batch_size)).to('cuda')
            val_metrics['seg']['Pixel Acc'] = (np.array(self.records['seg']['pixelAccs']) * np.array(batch_size)).sum() / sum(batch_size)
            # if 'module'in list(net.state_dict().keys())[0]:    
            #     dist.reduce(val_metrics_sp, 0, op=dist.ReduceOp.SUM)
            # val_metrics_sp /= int(num_gpu)
            # val_metrics['seg']['Pixel Acc'] = float(val_metrics_sp)

            # val_metrics_se = torch.tensor((np.array(self.records['seg']['errs']) * np.array(batch_size)).sum() / sum(batch_size)).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_metrics_se, 0, op=dist.ReduceOp.SUM)
            # val_metrics_se /= int(num_gpu)
            # val_metrics['seg']['err'] = float(val_metrics_se)
        ...
        if 'sn' in self.tasks:
            ...
            # val_am1 = torch.tensor(np.mean(angles)).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_am1, 0, op=dist.ReduceOp.SUM)
            # val_am1 /= int(num_gpu)

            # val_am2 = torch.tensor(np.median(angles)).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_am2, 0, op=dist.ReduceOp.SUM)
            # val_am2 /= int(num_gpu)

            # val_a1 = torch.tensor(np.mean(np.less_equal(angles, 11.25)) * 100).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_a1, 0, op=dist.ReduceOp.SUM)
            # val_a1 /= int(num_gpu)

            # val_a2 = torch.tensor(np.mean(np.less_equal(angles, 22.5)) * 100).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_a2, 0, op=dist.ReduceOp.SUM)
            # val_a2 /= int(num_gpu)

            # val_a3 = torch.tensor(np.mean(np.less_equal(angles, 30.0)) * 100).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_a3, 0, op=dist.ReduceOp.SUM)
            # val_a3 /= int(num_gpu)

            # val_a4 = torch.tensor(np.mean(np.less_equal(angles, 45.0)) * 100).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_a4, 0, op=dist.ReduceOp.SUM)
            # val_a4 /= int(num_gpu)

            # val_metrics['sn']['Angle Mean'] = float(val_am1)
            # val_metrics['sn']['Angle Median'] = float(val_am2)
            # val_metrics['sn']['Angle 11.25'] = float(val_a1)
            # val_metrics['sn']['Angle 22.5'] = float(val_a2)
            # val_metrics['sn']['Angle 30'] = float(val_a3)
            # val_metrics['sn']['Angle 45'] = float(val_a4)
        ...
        if 'depth' in self.tasks:
            ...
            # val_da = torch.tensor((self.records['depth']['abs_errs'] * np.array(batch_size)).sum() / sum(batch_size)).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_da, 0, op=dist.ReduceOp.SUM)
            # val_da /= int(num_gpu)

            # val_metrics['depth']['abs_err'] = float(val_da)

            # val_dr = torch.tensor((self.records['depth']['rel_errs'] * np.array(batch_size)).sum() / sum(batch_size)).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_dr, 0, op=dist.ReduceOp.SUM)
            # val_dr /= int(num_gpu)

            # val_metrics['depth']['rel_err'] = float(val_dr)

            # val_ds = torch.tensor(np.mean(np.less_equal(self.records['depth']['ratios'], 1.25)) * 100).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_ds, 0, op=dist.ReduceOp.SUM)
            # val_ds /= int(num_gpu)

            # val_metrics['depth']['sigma_1.25'] = float(val_ds)

            # val_ds2 = torch.tensor(np.mean(np.less_equal(self.records['depth']['ratios'], 1.25 ** 2)) * 100).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_ds2, 0, op=dist.ReduceOp.SUM)
            # val_ds2 /= int(num_gpu)

            # val_metrics['depth']['sigma_1.25^2'] = float(val_ds2)

            # val_ds3 = torch.tensor(np.mean(np.less_equal(self.records['depth']['ratios'], 1.25 ** 3)) * 100).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_ds3, 0, op=dist.ReduceOp.SUM)
            # val_ds3 /= int(num_gpu)

            # val_metrics['depth']['sigma_1.25^3'] = float(val_ds3)
        ...
        if 'keypoint' in self.tasks:
            ...
            # val_ke = torch.tensor((np.array(self.records['keypoint']['errs']) * np.array(batch_size)).sum() / sum(batch_size)).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_ke, 0, op=dist.ReduceOp.SUM)
            # val_ke /= int(num_gpu)

            # val_metrics['keypoint']['err'] = float(val_ke)
        ...
        if 'edge' in self.tasks:
            ...
            # val_ee = torch.tensor((np.array(self.records['edge']['errs']) * np.array(batch_size)).sum() / sum(batch_size)).to('cuda')
            # if 'module'in list(net.state_dict().keys())[0]:
            #     dist.reduce(val_ee, 0, op=dist.ReduceOp.SUM)
            # val_ee /= int(num_gpu)

            # val_metrics['edge']['err'] = float(val_ee)
